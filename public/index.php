...
function wallet_member_token(array $member): string {
    $id = (string)($member['id'] ?? '');
    $num = (string)($member['membership_number'] ?? '');
    return substr(hash_hmac('sha256', $id . '|' . $num, app_key()), 0, 24);
}

function wallet_member_verify_url(array $member): string {
    return app_base_url() . '/?route=wallet_verify&id=' . urlencode((string)$member['id']) . '&token=' . urlencode(wallet_member_token($member));
}

function wallet_qr_img(string $data, int $size=220): string {
    // Uses a public QR image endpoint for the admin preview/card page.
    // The QR payload is the member verification URL generated by this system.
    return 'https://api.qrserver.com/v1/create-qr-code/?size=' . (int)$size . 'x' . (int)$size . '&data=' . rawurlencode($data);
}
...
function wallet_member_payload(array $member): array {
    ...
    return [
        ...
        'licence_level' => $member['licence_level'] ?? '',
        'expires' => $member['renewal_date'] ?? '',
...
function render_physical_membership_card(array $member, bool $printLink=false): string {
    ...
    $html .= '<div class="physical-card-accent"></div>';
    $html .= '<div class="physical-card-header">';
    $html .= '<div class="physical-card-brand">' . membership_card_logo_html() . '<div class="physical-card-brand-copy"><strong>' . e($club) . '</strong><small>' . e($title) . ' card</small></div></div>';
    $html .= '<span class="physical-card-label">MEMBERSHIP</span>';
    $html .= '</div>';

    $html .= '<div class="physical-card-body">';
    $html .= '<div class="physical-card-details">';
    ...
    $html .= '<div class="physical-card-qr-panel">';
    $html .= '<div class="physical-card-qr-box"><img src="' . e($qr) . '" alt="Membership verification QR code"></div>';
    $html .= '<small class="physical-card-qr-caption">' . e($number ?: '') . '</small>';
    $html .= '</div>';
    $html .= '</div>';
    $html .= '</article>';
...
.physical-card-grid{gap:28px 22px}
.physical-card-item{align-content:start}
.physical-membership-card{
    ...
    overflow:hidden;
    background:
        radial-gradient(circle at 100% 0%, rgba(255,255,255,.10) 0 18mm, transparent 18.2mm),
        linear-gradient(180deg, color-mix(in srgb, var(--member-card-colour) 92%, #0f172a 8%) 0%, color-mix(in srgb, var(--member-card-colour) 86%, #0f172a 14%) 100%);
    color:#fff;
    box-shadow:0 18px 35px rgba(15,23,42,.15);
    padding:5.4mm 4.8mm 4.8mm;
    display:flex;
    flex-direction:column;
    justify-content:space-between;
}
.physical-card-accent{
    position:absolute;
    left:0; right:0;
    top:20.5mm;
    height:13.2mm;
...
.physical-card-brand-copy small{
    display:block;
    font-size:2.35mm;
    line-height:1.15;
    opacity:.92;
...
.physical-card-body{
    ...
    z-index:1;
    display:grid;
    grid-template-columns:minmax(0,1fr) 25.5mm;
    gap:4.5mm;
    align-items:end;
    margin-top:auto;
}
...
.physical-card-name strong{
    font-size:6.35mm;
    line-height:.98;
    font-weight:500;
}
.physical-card-fields-row{
    display:grid;
    grid-template-columns:1fr 1fr;
    gap:4mm;
    align-items:end;
}
.physical-card-fields-row .physical-card-field strong{
    min-height:4.5mm;
    font-size:4.15mm;
}
.physical-card-qr-panel{
    display:flex;
    flex-direction:column;
    align-items:center;
...
@media(max-width:720px){
    .physical-card-grid{gap:18px;padding:12px}
    .physical-card-actions .btn{min-width:unset;width:100%;max-width:320px}
}
...
if (route() === 'wallet_verify') {
    ...
    page_header('Verify membership card');
    if (!$m || !hash_equals(wallet_member_token($m), $token)) {
        echo '<div class="card danger"><h1>Invalid membership card</h1><p>This QR code could not be verified.</p></div>';
        page_footer(); exit;
    }
...
if (route() === 'wallet_card_json') {
    require_permission('manage_users');
    $id = (int)($_GET['id'] ?? 0);
    $m = first('SELECT * FROM members WHERE id=?', [$id]);
    if (!$m) { http_response_code(404); exit('Member not found'); }
    audit('wallet_card.json','member',$id);
    header('Content-Type: application/json');
    header('Content-Disposition: attachment; filename="membership-card-'.preg_replace('/[^A-Za-z0-9_-]+/','_',($m['membership_number'] ?: (string)$id)).'.json"');
    echo json_encode(wallet_member_payload($m), JSON_PRETTY_PRINT);
    exit;
}

if (route() === 'wallet_card_print') {
    require_permission('manage_users');
    $id = (int)($_GET['id'] ?? 0);
    $m = first('SELECT * FROM members WHERE id=?', [$id]);
    if (!$m) redirect('wallet_cards');
    audit('wallet_card.print','member',$id);
    $payload = wallet_member_payload($m);
    $qr = wallet_qr_img($payload['verify_url'], 260);
    page_header('Printable membership card');
    echo '<div class="wallet-print-wrap"><div class="wallet-pass-preview wallet-print-card"><div class="wallet-pass-top"><div><strong>'.e($payload['club']).'</strong></div><div>Membership</div></div><div class="wallet-pass-field"><span>MEMBER NAME</span><strong>'.e($payload['member_name']).'</strong></div><div class="wallet-pass-grid"><div><span>MEMBERSHIP NUMBER</span><strong>'.e($payload['membership_number'] ?: 'Not set').'</strong></div><div><span>EXPIRES</span><strong>'.e($payload['expires'] ?: 'Not set').'</strong></div><div><span>CALLSIGN</span><strong>'.e($payload['callsign'] ?: 'Not set').'</strong></div><div><span>STATUS</span><strong>'.e($payload['status'] ?: 'Unknown').'</strong></div></div><div class="wallet-qr-box"><img src="'.e($qr).'" alt="QR code"><small>'.e($payload['membership_number'] ?: '').'</small></div></div></div><script>window.print();</script>';
    page_footer(); exit;
}
...
if (route() === 'wallet_cards') {
    ...
    foreach($members as $m){
        ...
        if (!empty($m['membership_number'])) $label .= ' / '.$m['membership_number'];
        echo '<option value="'.e($m['id']).'" '.($selected && (int)$selected['id']===(int)$m['id']?'selected':'').'>'.e($label).'</option>';
    }
    ...
    echo '<div class="card wallet-preview-card"><h2>Card preview</h2>';
    if (!$selected) {
        echo '<p class="muted">No member selected.</p>';
    } else {
        $qr = wallet_qr_img($payload['verify_url'], 220);
        echo '<div class="wallet-pass-preview"><div class="wallet-pass-top"><div><strong>'.e($payload['club']).'</strong></div><div>Membership</div></div><div class="wallet-pass-field"><span>MEMBER NAME</span><strong>'.e($payload['member_name']).'</strong></div><div class="wallet-pass-grid"><div><span>MEMBERSHIP NUMBER</span><strong>'.e($payload['membership_number'] ?: 'Not set').'</strong></div><div><span>EXPIRES</span><strong>'.e($payload['expires'] ?: 'Not set').'</strong></div><div><span>CALLSIGN</span><strong>'.e($payload['callsign'] ?: 'Not set').'</strong></div><div><span>STATUS</span><strong>'.e($payload['status'] ?: 'Unknown').'</strong></div></div><div class="wallet-qr-box"><img src="'.e($qr).'" alt="QR code"><small>'.e($payload['membership_number'] ?: '').'</small></div></div><p class="muted">QR verifies this member against the system: <code>'.e($payload['verify_url']).'</code></p>';
    }
    echo '</div></div>';

    echo '<div class="card"><h2>Recommended next step</h2><p>Next we can add upload fields for Apple pass certificates and Google Wallet issuer credentials, then enable the Apple/Google download buttons above.</p></div>';
