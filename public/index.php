...
function attendance_stats(int $member_id): array {
    // Count DISTINCT events rather than raw attendance rows. Legacy imports can create
    // duplicate rows per event/member, which previously allowed impossible values
    // such as 15000%. Percentages are capped at 100%.
    $signed = (int)(first('SELECT COUNT(DISTINCT event_id) c FROM event_attendance WHERE member_id=? AND status IN ("signed_up","attended","did_not_attend")',[$member_id])['c'] ?? 0);
    $att = (int)(first('SELECT COUNT(DISTINCT event_id) c FROM event_attendance WHERE member_id=? AND attended=1',[$member_id])['c'] ?? 0);
    $events = (int)(first('SELECT COUNT(*) c FROM events WHERE visibility="members"',[])['c'] ?? 0);
    if ($att > $signed) $signed = $att;
    return [
        'signed_up' => $signed,
        'attended' => $att,
        'signup_percent' => $signed ? min(100, round($att/$signed*100,1)) : null,
        'eligible_events' => $events,
        'overall_percent' => $events ? min(100, round($att/$events*100,1)) : null,
    ];
}
...
if (route() === 'members') {
    ...
    if (!$rows) {
        ...
        foreach($rows as $m){
            ...
            echo '<article class="member-card-modern"><div class="member-card-main"><div class="member-avatar">'.e($initials).'</div><div><div class="member-card-top"><h3>'.e($name ?: 'Unnamed member').'</h3><span class="status-pill status-'.e($statusClass).'">'.e($m['membership_status'] ?: 'unknown').'</span></div><p class="muted">'.e($m['callsign'] ?: 'No callsign').' • Member no. '.e($m['membership_number'] ?: 'not set').'</p><div class="member-card-meta"><span>Joined</span><strong>'.e(member_joined_display($m)).'</strong><span>Renewal</span><strong>'.e($m['renewal_date'] ?: 'Not set').'</strong><span>Email</span><strong>'.e($m['email'] ?: 'Not set').'</strong></div><div class="member-attendance"><div><span>Attendance</span><strong>'.e($pctText).'</strong></div><div class="progressbar"><span style="width:'.e($pctWidth).'%"></span></div></div></div></div><div class="member-card-actions"><a class="btn secondary" href="?route=member_view&id='.e($m['id']).'">Open</a>';
