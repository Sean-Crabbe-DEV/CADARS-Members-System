...
function page_header(string $title): void {
    ...
    if ($u) {
        ...
        echo '<a href="?route=dashboard">Dashboard</a><a href="?route=events">Programme</a><a href="?route=brickworks">Brickworks</a><a href="?route=directory">Directory</a>';
        if (is_committee_or_admin()) {
            ...
            echo '<a href="?route=members">Members</a><a href="?route=users">Users</a><a href="?route=attendance">Attendance tracking</a><a href="?route=audit">Audit logs</a><a href="?route=emails">Emails</a>';
            echo '</div></div>';
        }
...
if (route() === 'event_view') {
    ...
    if ($_SERVER['REQUEST_METHOD']==='POST') { require_csrf();
        ...
        if (isset($_POST['delete_event'])) { if (!can_manage_events()) require_permission('manage_events'); exec_sql('DELETE FROM event_attendance WHERE event_id=?',[$id]); exec_sql('DELETE FROM event_attachments WHERE event_id=?',[$id]); exec_sql('DELETE FROM events WHERE id=?',[$id]); audit('event.delete','event',$id); flash('Event deleted.'); redirect('events'); }
        if (isset($_POST['mark_attendance'])) { require_permission('track_attendance'); foreach(($_POST['attended'] ?? []) as $memberId=>$val){ exec_sql('UPDATE event_attendance SET attended=?, status=CASE WHEN ?=1 THEN "attended" ELSE "did_not_attend" END, marked_at=datetime("now"), marked_by_user_id=?, updated_at=datetime("now") WHERE event_id=? AND member_id=?',[(int)$val,(int)$val,$u['id'],$id,(int)$memberId]); } audit('attendance.update','event',$id); flash('Attendance updated.'); redirect('event_view&id='.$id); }
    }
    ...
    if (is_committee_or_admin()) {
        ...
        echo '<div class="card"><h2>Attendance</h2><form method="post" class="attendance-tools">'.csrf_field().'<input type="hidden" name="add_member_attendance" value="1"><div><label>Add member to attendance list</label><select name="member_id"><option value="">Select member...</option>'; foreach($available as $m) echo '<option value="'.e($m['id']).'">'.e($m['first_name'].' '.$m['last_name'].($m['callsign']?' - '.$m['callsign']:'')).'</option>'; echo '</select></div><button>Add</button></form>';
        if(!$att) echo '<p>No one has signed up yet. Add members above to track attendance manually.</p>'; else { echo '<form method="post">'.csrf_field().'<input type="hidden" name="mark_attendance" value="1"><table><tr><th>Member</th><th>Status</th><th>Attended?</th><th>Signed up/added</th></tr>'; foreach($att as $a){ echo '<tr><td>'.e($a['first_name'].' '.$a['last_name'].' '.$a['callsign']).'</td><td>'.e($a['status']).'</td><td><select name="attended['.e($a['member_id']).']"><option value="1" '.($a['attended']==='1'?'selected':'').'>Yes</option><option value="0" '.($a['attended']==='0'?'selected':'').'>No</option></select></td><td>'.e($a['signed_up_at']).'</td></tr>'; } echo '</table><p><button>Save attendance</button></p></form>'; } echo '</div>';
    }
    page_footer(); exit;
}
...
if (route() === 'attendance') {
    ...
    $events=all('SELECT e.*,COUNT(a.id) signups,COUNT(CASE WHEN a.attended=1 THEN 1 END) attended FROM events e LEFT JOIN event_attendance a ON a.event_id=e.id GROUP BY e.id ORDER BY e.start_at DESC');
    echo '<div class="card"><h1>Attendance tracking</h1><table><tr><th>Event</th><th>Date</th><th>Signed up</th><th>Attended</th><th>Action</th></tr>'; foreach($events as $ev) echo '<tr><td>'.e($ev['title']).'</td><td>'.e($ev['start_at']).'</td><td>'.e($ev['signups']).'</td><td>'.e($ev['attended']).'</td><td><a class="btn secondary" href="?route=event_view&id='.e($ev['id']).'">Open attendance</a></td></tr>'; echo '</table></div>'; page_footer(); exit;
}
...
if (route() === 'emails') {
    ...
    if ($_SERVER['REQUEST_METHOD']==='POST') { require_csrf();
        ...
        $members=all('SELECT m.*,ep.allow_open_tracking FROM members m LEFT JOIN member_email_preferences ep ON ep.member_id=m.id JOIN member_consents c ON c.member_id=m.id AND c.consent_type="email_comms" AND c.granted=1 WHERE m.membership_status="active"');
