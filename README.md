...
Added an Admin > Wallet cards page. Admins can select a member, preview a wallet-style membership card, print/save it, and download the pass data JSON. The card includes a QR verification URL generated by the membership system. Real Apple Wallet .pkpass support still requires an Apple Pass Type ID and signing certificate; Google/Android Wallet support requires Google Wallet issuer/API credentials.
